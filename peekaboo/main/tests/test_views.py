...
class TestViews(TestCase):
    ...
    def test_log_entries(self):
        ...
        data = json.loads(response.content)
        eq_(len(data['rows']), 1)
        eq_(data['rows'][0]['name'], 'Bill Gates')
        eq_(data['rows'][0]['job_title'], 'Boss')
        eq_(data['rows'][0]['id'], visitor1.pk)
        ...
        data = json.loads(response.content)
        eq_(len(data['rows']), 1)
        eq_(data['rows'][0]['name'], 'Paul Allen')
        eq_(data['rows'][0]['id'], visitor2.pk)
